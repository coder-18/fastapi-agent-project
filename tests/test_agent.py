...
def test_get_llm():
    ...
    with patch("app.agent.OpenAI", return_value=mock_llm) as mock_openai_class:
        result = get_llm(openai_api_key)
        mock_openai_class.assert_called_once_with(openai_api_key=openai_api_key)
...
def test_get_agent(mock_load_openai_api_key, mock_get_llm, mock_initialize_agent):
